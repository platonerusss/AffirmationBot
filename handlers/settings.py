...
from database import get_user_settings, set_user_enabled, set_user_time, set_user_mode
...
def make_text_settings(settings):
    ...
    mode = get_mode_text(settings["mode"])
...
@router.callback_query(F.data == "mode_female")
async def mode_female_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    ...
    await callback.message.edit_text(text, reply_markup=settings_keyboard)
    await callback.answer("Включён женский режим")


@router.callback_query(F.data == "mode_male")
async def mode_male_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
